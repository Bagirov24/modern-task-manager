...
export default function ActionInboxPage() {
  ...
  const [status, setStatus] = useState<(typeof groups)[number]['value']>('all')
  ...
  const inbox = useQuery({
    queryKey: ['communication-items', status, search],
    queryFn: async () => (await communicationApi.list({ action_status: status === 'all' ? undefined : status, search: search || undefined, per_page: 100 })).data,
  })
  ...
  return <Stack spacing={2.5} sx={{ maxWidth: 1400, mx: 'auto' }}>
    ...
    <Paper variant="outlined" sx={{ p: 1.5 }}><Stack gap={1.25}><TextField size="small" placeholder="Поиск по отправителю, теме и содержанию" value={search} onChange={(event) => setSearch(event.target.value)} InputProps={{ startAdornment: <InputAdornment position="start"><Search /></InputAdornment> }} /><Stack direction="row" gap={0.75} flexWrap="wrap">{groups.map((group) => <Chip key={group.value} clickable color={status === group.value ? 'primary' : 'default'} variant={status === group.value ? 'filled' : 'outlined'} label={`${group.label}${group.value !== 'all' && inbox.data?.groups[group.value] ? ` ${inbox.data.groups[group.value]}` : ''}`} onClick={() => setStatus(group.value)} />)}</Stack></Stack></Paper>
    {inbox.isError && <Alert severity="error">Не удалось загрузить входящие действия.</Alert>}
    {!inbox.isLoading && !inbox.data?.items.length ? <Paper variant="outlined" sx={{ py: 8, textAlign: 'center' }}><ForumOutlined sx={{ fontSize: 44, color: 'text.disabled' }} /><Typography variant="h6" fontWeight={700} mt={1}>Входящих действий нет</Typography><Typography color="text.secondary">Добавляйте только сообщения, где действительно требуется действие.</Typography></Paper> : <Stack spacing={1}>{inbox.data?.items.map((item) => <InboxCard key={item.id} item={item} projectName={item.project_id ? projectMap.get(item.project_id) : undefined} onStatus={(value) => update.mutate({ item, values: { action_status: value } })} onCreateTask={() => createTask.mutate(item)} />)}</Stack>}
...
function InboxCard({ item, projectName, onStatus, onCreateTask }: { item: CommunicationItem; projectName?: string; onStatus: (value: CommunicationActionStatus) => void; onCreateTask: () => void }) {
  ...
  return <Paper variant="outlined" sx={{ p: 2, borderLeft: '4px solid', borderLeftColor: overdue ? 'error.main' : item.importance === 'critical' ? 'warning.main' : 'divider' }}>
