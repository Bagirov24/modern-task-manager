...
export default function FocusNowCard({ focus, candidates, state }: FocusNowCardProps) {
  const setPinnedFocusEntityKey = useUIStore((store) => store.setPinnedFocusEntityKey)
    ...
    <Paper component="section" aria-labelledby="focus-now-heading" variant="outlined" sx={{ p: { xs: 2, md: 2.5 }, borderLeft: '4px solid', borderLeftColor: selected?.isBlocked ? 'error.main' : 'primary.main' }}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" gap={2}>
        <Box sx={{ minWidth: 0, flex: 1 }}>
          ...
          <AttentionState loading={state.loading} error={state.error} onRetry={state.retry} empty={!selected} emptyTitle="Фокус не выбран" emptyDescription="В очереди пока нет доступных действий.">
            {selected && <Stack spacing={1.25} sx={{ mt: 1.5 }}>
              <Typography variant="h6" fontWeight={750}>{selected.title}</Typography>
              <Stack direction="row" gap={1} flexWrap="wrap">
                <ActionStatusChip item={selected} />
      ...
      <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)} MenuListProps={{ role: 'listbox', 'aria-label': 'Выбор фокуса' }}>
        {candidates.map((item) => <MenuItem key={item.entityKey} role="option" aria-selected={item.entityKey === selected?.entityKey} selected={item.entityKey === selected?.entityKey} onClick={() => choose(item)}>{item.title}</MenuItem>)}
      </Menu>
    </Paper>
  )
}
