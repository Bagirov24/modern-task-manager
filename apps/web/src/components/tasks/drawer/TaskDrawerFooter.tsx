...
export default function TaskDrawerFooter({ hasTask, saving, saveDisabled, onDelete, onCancel, onSave }: Props) {
  return <Stack direction="row" justifyContent="space-between" sx={{ px: 2, py: 1.25, borderTop: '1px solid', borderColor: 'divider' }}>
    {hasTask ? <Button color="error" startIcon={<DeleteOutline />} onClick={onDelete}>Удалить</Button> : <Box />}
    <Stack direction="row" spacing={1}>
      <Button onClick={onCancel}>Отмена</Button>
      <Button variant="contained" startIcon={saving ? <CircularProgress size={16} color="inherit" /> : <Save />} disabled={saveDisabled || saving} onClick={onSave}>Сохранить</Button>
    </Stack>
  </Stack>
}
